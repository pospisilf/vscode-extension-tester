...
export class ScreenshotsTreeProvider implements vscode.TreeDataProvider<ScreenshotsResourcesItem> {
    ...
    private resolveScreenshotsPath(): string {
            ...
            if (baseTempDir && baseTempDir.length > 0) {
                baseTempDir = path.isAbsolute(baseTempDir)
                    ? baseTempDir
                    : path.join(workspaceFolder || '', baseTempDir);
            } else {
...
class ScreenshotsResourcesItem extends vscode.TreeItem {
    ...
    constructor(public readonly label: string, public readonly filePath: string) {
        ...
        if (filePath) {
            ...
            // Placeholder without file
            this.iconPath = new vscode.ThemeIcon('warning'); // maybe info better? 
        }

        // this.tooltip = filePath;
        // this.resourceUri = vscode.Uri.file(filePath); // Enables file icon support
        // this.iconPath = vscode.ThemeIcon.File; // Uses VS Code's file icon
