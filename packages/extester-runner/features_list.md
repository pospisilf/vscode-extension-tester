...
- Screenshots (Tree View ID: `extesterResourcesScreenshotsView`): Shows screenshot image files (likely from test runs).
...
- `outFolder` (string): Path to the compiled output directory for tests. If not set, the extension will default to the `outDir` from `tsconfig.json` or `'out'` as fallback.
- `rootFolder` (string): Path to the test source root. If not set, uses `rootDir` from tsconfig.json or falls back to none (tests run from workspace root).
...
- `TSConfig` (string): Name/path of the tsconfig file to use (default `tsconfig.json`).
...
- RunFileTask: Runs tests from a single file. It calculates the output `.js` file path corresponding to the source `.ts` file (using `rootFolder/outFolder` settings and `tsconfig.json` to map the path) and then runs `npx extest setup-and-run '<compiled-file-path>' [options]`.
...
The extension contributes a “Get started with ExTester Runner” walkthrough with steps guiding the user to configure `tsconfig.json`, `Root/Out` folders, etc. (While this is a feature, it likely doesn’t need automated testing beyond verifying its existence.)
